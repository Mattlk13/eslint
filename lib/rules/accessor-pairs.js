...
/** @typedef {import("./utils/rule").Node<"ObjectExpression">} ObjectExpression */
 ...
 * @param {ObjectExpression} node - A node to check.
...
module.exports = rule({
    ...
    create(context) {
         ...
         * @param {ObjectExpression} node The node to check.
         * @returns {void}
         * @private
         */
            ...
            for (let i = 0, end = node.properties.length; i < end; i++) {
                ...
                if (property.kind === "init") {
                    if (isDescriptor && !property.computed) {
                        propToCheck = property.key.type === "Identifier"
                            ? property.key.name
